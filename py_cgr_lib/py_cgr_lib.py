    ...
    with open(file_name, 'r') as cf:
        for contact in cf.readlines():
            ...
            if contact[0] == '#':
                continue
            if contact[0] == '@':
                add_inteval += int(contact.split()[1])
                continue
            ...
            for i, contact in enumerate(route.get_hops()):
                if i == 0:
                    f.write("{},{},true,{},'{},{},{}',{}\n".format('root', contact_plan.index(contact),
                                                                   weight, 0, 0, 0, route.best_delivery_time))
                if i > 0:
...
def fwd_candidate(curr_time, curr_node, contact_plan, bundle, routes, excluded_nodes):
    ...
    for route in routes:
        ...
        #输出排队信息
        print(f"applicable_backlog_relief is: {applicable_backlog_relief}")
        if residual_backlog > 0 :
            print(f"route: {route}'s applicable_backlog_p is:", applicable_backlog_p)
