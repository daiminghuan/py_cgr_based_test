...
# print(contact_plan)
contact_plan = cp_random(3184, 48)
source = contact_plan[0].frm
destination = contact_plan[-1].to
print("---yen---")
for curr_time in range(0, 86400, 50):
    ...
    print("start calling..." )
    routes = cgr_yen(source, destination, curr_time, contact_plan, 2)
    ...
    print("---candidate---")
    excluded_nodes = []
    bundle = Bundle(src=source, dst=destination, size=125, deadline=72000, priority=0)
